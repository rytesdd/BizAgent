...
import AiAssistantSidebar from '../experiments/AiAssistantSidebarV3'; // Use V3 specific version
...
import MobileBottomSheet from '../components/MobileBottomSheet'; // New Step 2 Component
...
const VERSIONS_STORAGE_KEY = 'dualrole_v3_versions';
...
export default function DualRoleViewV3() {
    // --- Infrastructure Check ---
    const isMobile = useMediaQuery('(max-width: 768px)');

    // --- Step 3: Progressive Layout State Machine ---
    // 'chat' = Chat Only (Center/Full)
    // 'split' = Chat + Doc (Sidebar + Main)
    // 'comments' = Chat + Doc + Comments (3-Col)
    const [viewStage, setViewStage] = useState('chat');

    // Mobile View Stack: 'chat' | 'document' | 'comments_sheet'
    const [mobileView, setMobileView] = useState('chat');
    const [isSheetOpen, setIsSheetOpen] = useState(false);

    useEffect(() => {
        console.log(`📱 [DualRoleV3] Mode: ${isMobile ? 'Mobile' : 'Desktop'} | Stage: ${viewStage} | MobileView: ${mobileView}`);
    }, [isMobile, viewStage, mobileView]);
...
`;
    ...
    const handleElementClick = (targetId) => {
        // Stop bubbling/default only if needed (removed to allow normal selection)

        console.log("🖱️ [Interaction] User clicked Document Element:", targetId);

        // Mobile Logic: If in Document view, open sheet
        if (isMobile) {
            setIsSheetOpen(true);
        } else {
            // Desktop Logic: Enhance to 'comments' view stage
            if (viewStage !== 'comments') setViewStage('comments');
        }
        ...
        if (foundComment) {
            ...
            // Wait for sheet/sidebar to render
            setTimeout(() => {
    ...
    // Callback when Sidebar clicks a Gateway/Document Card
    // Triggers layout transition: Chat Only -> Split View
    const handleWidgetClick = (type, data) => {
        console.log('🖱️ [Widget Click]', type, data);

        // If it's a document/file card, open the document view
        if (type === 'file' || type === 'document' || (data && data.type === 'document')) {
            if (isMobile) {
                setMobileView('document');
            } else {
                if (viewStage === 'chat') setViewStage('split');
            }
        }
    };
    ...
    return (
        <div className="absolute inset-4 flex flex-col text-white font-sans overflow-hidden gap-4">
            ...
            {/* ========================================== */}
            <div className="h-14 w-full flex items-center justify-between px-5 bg-[#131314] shrink-0">
                <div className="flex items-center gap-3">
                    <span className="font-bold text-base text-zinc-100">Dual-Role V3 Prototype {isMobile && '(Mobile)'}</span>

                    {/* ROLE SWITCHER */}
                    <div className="flex bg-zinc-800/50 rounded-lg p-0.5 ml-4">
                        <button
            ...
            {/* ========================================== */}
            <div className={`flex-1 flex justify-center gap-4 min-h-0 overflow-hidden relative transition-all duration-500 ease-out`}>

                {/* --- Column 1: AI Assistant Sidebar --- */}
                {/* Desktop: Dynamic Width based on ViewStage */}
                {/* Mobile: Full Screen if mobileView === 'chat' */}
                <div className={`
                    bg-[#131314] flex flex-col transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)]
                    ${isMobile
                        ? (mobileView === 'chat' ? 'absolute inset-0 z-10 w-full translate-x-0' : 'absolute inset-0 -translate-x-full opacity-0')
                        : (viewStage === 'chat'
                            ? 'w-full max-w-[800px] mx-auto translate-x-0 pt-10'  // Added top padding for breathing room
                            : 'w-[380px] shrink-0 translate-x-0') // Removed border-r
                    }
                `}>
                    <AiAssistantSidebar
                        ref={sidebarRef}
                        ...
                        // Connect Widget Click -> Layout Transition
                ...
                <div className={`
                    flex-1 relative overflow-hidden min-w-0 bg-[#2C2C2C] rounded-xl flex flex-col
                    transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)]
                    ${isMobile
                        ? (mobileView === 'document' ? 'absolute inset-0 z-20 translate-x-0' : 'absolute inset-0 translate-x-full')
                        : (viewStage === 'chat'
                            ? 'translate-x-[120%] opacity-0 absolute right-0 w-1/2 h-full' // Hidden offscreen
                            : 'translate-x-0 opacity-100 relative'
                        )
                    }
                `}>
                    {/* Mobile Back Button Overlay */}
                    {isMobile && mobileView === 'document' && (
                        <div className="absolute top-4 left-4 z-50">
                            <button
                                onClick={() => setMobileView('chat')}
                                className="bg-black/50 backdrop-blur-md text-white p-2 rounded-full shadow-lg border border-white/10 active:scale-95"
                            >
                ...
                {/* Desktop: Slide In from Right when viewStage === 'comments' */}
                {/* Mobile: Use BottomSheet instead */}
                {!isMobile && (
                    <div className={`
                        w-[340px] bg-zinc-900 flex flex-col rounded-xl overflow-hidden
                        transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)]
                        ${viewStage === 'comments'
                            ? 'translate-x-0 opacity-100 mr-0'
                            : 'translate-x-[100%] opacity-0 w-0 -mr-[340px] pointer-events-none'
                        }
                    `}>
                        <div className="h-14 flex items-center justify-between px-4 bg-zinc-900/50">
                            ...
                            <div className="flex items-center gap-2">
                                <button
                                    onClick={() => setViewStage('split')} // Close comments
                ...
                >
                    <div className="space-y-4 pb-10">
                        {/* Step 2: 评论总结按钮 - 仅乙方可见 (Mobile) */}
                        {currentRole === 'PARTY_B' && comments.filter(isHumanClientComment).length > 0 && (
                            <button
                                onClick={handleSummarizeComments}
                                disabled={isSummarizing}
                                className={`w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-medium transition-all ${isSummarizing
                                    ? 'bg-zinc-800 text-zinc-500 cursor-wait'
                                    : 'bg-blue-600/20 text-blue-400 active:scale-95'
                                    }`}
                            >
                        ...
                        {/* Helper: Manual Close */}
                        {comments.length === 0 && (
                            <div className="text-center text-zinc-500 py-10">暂无评论</div>
                        )}
